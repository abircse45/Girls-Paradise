...
class ApiService {
  Future<List<ProductElement>> searchProducts(int categoryId) async {
    try {
      final response = await http.get(
        Uri.parse(
            'https://girlsparadisebd.com/api/v1/search_shop?category=${categoryId}&min=&max=&filter='),
      );
...
class _ProductScreenState extends State<ProductScreen> {
  ...
  Future<void> toggleWishlist(String productId, ProductElement productData) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      final products = await _apiService.searchProducts(widget.categoryId);
      setState(() {
        _products = products;
        _isLoading = false;
      });
