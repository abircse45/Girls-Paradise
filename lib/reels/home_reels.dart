...
class _HomeReelsState extends State<HomeReels> {
  ...
  Future<void> _loadReels() async {
    ...
    try {
      final reels = await _apiService.fetchReels();
      final validReels = reels.where((r) => r.isFacebookVideo).toList();
      if (validReels.isEmpty) {
        setState(() {
          _error = "No valid Facebook videos found";
          _isLoading = false;
        });
        return;
      }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async => await _loadReels(),
      child: Container(
        child: _isLoading
            ? const Center(child: CircularProgressIndicator(color: Colors.white))
            : _error != null
            ? Center(child: Text(_error!, style: const TextStyle(color: Colors.black)))
            : Stack(
          children: [
            PageView.builder(
              scrollDirection: Axis.vertical,
              ...
              itemBuilder: (context, index) {
                return GestureDetector(
                  onTap: () {}, // optional pause/play gesture
                  child: Container(
                    color: Colors.black,
                    child: Stack(
                      fit: StackFit.expand,
                          ...
                          child: SizedBox(
                            ...
                            child: InAppWebView(
                              initialUrlRequest: URLRequest(
                                url: WebUri(_reels[index].embedUrl),
                              ),
                            ...
                            children: [
                              GestureDetector(
                                onTap: () {
                                  final currentReel = _reels[_currentIndex];
                                  Get.to(
                                    ProductDetails(id: currentReel.id),
                                    transition: Transition.noTransition,
                                  );
                                },
                                child: Container(
                              ...
                              const SizedBox(height: 20),
                              _buildMessengerIcon("assets/images/messenger.png"),
                              const SizedBox(height: 16),
                              _buildIconButton(
                                  Icons.share_outlined,
                                  '${_reels[_currentIndex].slug}'),
                            ],
                          ),
                        ),
                      ],
  ...
  Widget _buildMessengerIcon(String path) {
    return Column(
      children: [
        GestureDetector(
          onTap: shareProduct,
